...
class NaiveClassSampler(Sampler):
    def __init__(
        ...
        **kwargs
        ...
        for example_index, targets in enumerate(data.select(pl.col(TARGET_COLUMN))):
...
class BySequenceLengthSampler(Sampler):
    def __init__(
        ...
        if isinstance(data, list):
            for example_index, (token_ids, _, _, _, _) in enumerate(data):
                ...
                self.ind_n_len.append((example_index, num_tokens))
        else:
            for example_index, row in enumerate(data.iter_rows(named=True)):
    ...
    def __iter__(self):
        ...
        for batch in iter_list:
            yield batch.numpy().tolist()

    def __len__(self):
        return (self.data_len // self.batch_size) + 1 - int(self.drop_last)

    def element_to_bucket_id(self, x, seq_length):
        """Assign a sequence length to a bucket ID."""
